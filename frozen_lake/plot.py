        ...
        for i in range( self.nRows ):
            for j in range( self.nColumns ):
                if self.maze[i][j] > 0 and self.maze[i][j] != 1:
                    self.screen.blit( self.sprites[int( self.maze[i][j] ) - 1],
                                      (j * self.cellWidth, i * self.cellHeight) )

        if self.playerPos != -1:
            self.screen.blit( self.sprites[0], ((self.playerPos % self.nColumns) * self.cellWidth,
